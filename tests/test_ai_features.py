...
def clear_active_optimizations(db):
    """Fixture to clear active optimizations and ensure a clean DB state."""
...
async def test_get_ab_test_results_with_trades(async_client):
    ...
    async with async_client as client:
        ...
        results = response.json()
        assert results["test_name"] == test_name
        assert results["variant_a_trades"] == 10
        assert results["variant_b_trades"] == 15
