...
const TransformerTrainer = ({ onModelTrained, trainedModels }) => {
  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/v2/deep-learning/transformer/train`, {
        method: 'POST',
  ...
  const handlePredict = async (ticker) => {
    try {
      ...
      const result = await response.json();
      alert(`Transformer Prediction for ${ticker}:\n${JSON.stringify(result.prediction, null, 2)}`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      {/* Configuration Form */}
      <Card>
        <h3 className="text-lg font-semibold mb-4">Transformer Configuration</h3>
        ...
        <div className="mt-6">
          ...
          >
            {training ? 'Training...' : '🔮 Train Transformer Model'}
          </button>
        </div>
      </Card>
      ...
      {/* Training Status */}
      {training && (
        <LoadingCard message="Training Transformer model... This may take several minutes." />
      )}
...
export default TransformerTrainer;
