// === PineLab Dashboard + Walk-Forward Integration + QoL Enhancements ===
// Adds automatic walk-forward validation wiring and a “Use Best Params” button in the Strategy Studio.

import React, { useState, useEffect } from 'react';
import { LineChart, Line, CartesianGrid, XAxis, YAxis, Tooltip, ResponsiveContainer, Legend, BarChart, Bar } from 'recharts';
import PineLabEasyDashboard from './PineLabEasyDashboard';
import BrokerPanel from './BrokerPanel';
import PineLabAdvancedSecurity from './PineLabAdvancedSecurity';

const NavButton = ({ active, label, onClick }: any) => (
  <button onClick={onClick} className={`px-3 py-2 rounded-xl text-sm font-medium ${active ? 'bg-black text-white' : 'bg-gray-200 hover:bg-gray-300'}`}>{label}</button>
);
...
const MetricCard = ({ title, value, change, good }: any) => (
  <div className="flex flex-col p-4 bg-white rounded-xl border shadow-sm">
    ...
    {change && (
      <div className={`text-xs ${good ? 'text-green-600' : 'text-red-500'}`}>{change}</div>
    )}
  </div>
);

export default function PineLabUnifiedDashboard({ apiBase }: { apiBase: string }) {
  const [tab, setTab] = useState<'studio'|'broker'|'security'>('studio');
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const newSummary = { equity: eq, cash, unrealized: unreal, pnlPct, ordersToday: today, health: pnlPct > -1 ? 'Good' : 'Review' };
        setSummary(newSummary);
        const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        setChartData((prev: any) => [...prev.slice(-30), { time: timestamp, equity: eq, pnl: pnlPct }]);
      } catch {}
    };
    fetchSummary();
  ...
  useEffect(() => {
    const pollMetrics = async () => {
      try {
        ...
        setAutoTuneStatus(s);
        setAutoTuneOn(s?.enabled || false);
        if (s?.promoted?.params) setBestParams(s.promoted.params);
      } catch {}
    };
    pollMetrics();
    const t = setInterval(pollMetrics, 15000);
    return () => clearInterval(t);
  }, [apiBase]);

  const startAutoTune = async () => {
    const fd = new FormData();
    Object.entries(autoTuneCfg).forEach(([k,v])=>fd.append(k,String(v)));
    await fetch(`${apiBase}/autotune/start_bayes`, { method: 'POST', body: fd });
    const s = await fetch(`${apiBase}/autotune/status`).then(r=>r.json());
    setAutoTuneStatus(s); setAutoTuneOn(true);
  };

  const stopAutoTune = async () => {
    await fetch(`${apiBase}/autotune/stop`, { method: 'POST' });
    const s = await fetch(`${apiBase}/autotune/status`).then(r=>r.json());
    setAutoTuneStatus(s); setAutoTuneOn(false);
  };

  const promoteBest = async () => {
    await fetch(`${apiBase}/autotune/promote_best`, { method: 'POST' });
    alert('Best parameters promoted to active strategy!');
  };

  const useBestParams = async () => {
    try {
      const best = await fetch(`${apiBase}/strategy/params/best`).then(r=>r.json());
      if (!best?.params) return alert('No promoted parameters found.');
      setBestParams(best.params);
      alert('Loaded promoted best parameters into Strategy Studio.');
    } catch {
      alert('Error loading best parameters.');
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white border rounded-xl shadow-sm p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {!autoTuneOn ? (
              <button onClick={startAutoTune} className="px-3 py-2 rounded-lg bg-black text-white text-xs">Start</button>
            ) : (
      ...
      {bestParams && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-3 text-xs text-gray-800">
      ...
      <div className="bg-white border rounded-xl shadow-sm p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-semibold">A/B Live Shadow Deployments</h3>
        </div>
...
function ABLiver({ apiBase }: { apiBase: string }) {
  ...
  const startAB = async () => {
    const fd = new FormData();
    fd.append('control', cfg.control);
    fd.append('shadow', cfg.shadow);
    fd.append('window_mins', String(cfg.windowMins));
    fd.append('promote_sharpe_delta', String(cfg.promoteIf.sharpeDelta));
    fd.append('promote_dd_cap', String(cfg.promoteIf.ddCap));
    await fetch(`${apiBase}/ab/start`, { method: 'POST', body: fd });
    const s = await fetch(`${apiBase}/ab/status`).then(r=>r.json());
    setStatus(s); setAbOn(true);
  };

  const stopAB = async () => {
    await fetch(`${apiBase}/ab/stop`, { method: 'POST' });
    const s = await fetch(`${apiBase}/ab/status`).then(r=>r.json());
    setStatus(s); setAbOn(false);
  };

  const promote = async () => {
    await fetch(`${apiBase}/ab/promote_candidate`, { method: 'POST' });
    alert('Candidate promoted to control.');
  };

  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <label className="text-xs text-gray-600">Control Broker</label>
        <select className="border rounded px-2 py-1 text-xs" value={cfg.control} onChange={e=>setCfg({...cfg, control: e.target.value})}>
          <option value="live">Live</option>
          <option value="paper">Paper</option>
        </select>
        <label className="text-xs text-gray-600">Shadow Broker</label>
        <select className="border rounded px-2 py-1 text-xs" value={cfg.shadow} onChange={e=>setCfg({...cfg, shadow: e.target.value})}>
          <option value="paper">Paper</option>
          <option value="live">Live</option>
        </select>
        ...
        <label className="text-xs text-gray-600">Promote if SharpeΔ ≥</label>
        ...
        <label className="text-xs text-gray-600">and DD ≤</label>
        ...
        {!abOn ? (
          <button onClick={startAB} className="px-3 py-2 rounded-lg bg-black text-white text-xs">Start A/B</button>
        ) : (
      ...
      <div className="grid md:grid-cols-2 gap-3 text-xs">
        <div className="bg-gray-50 rounded-lg p-3 border">
          <div className="font-semibold mb-1">Control (Current Live)</div>
          <pre className="whitespace-pre-wrap">{JSON.stringify(status.control || {}, null, 2)}</pre>
        </div>
        <div className="bg-gray-50 rounded-lg p-3 border">
          <div className="font-semibold mb-1">Candidate (Shadow)</div>
          <pre className="whitespace-pre-wrap">{JSON.stringify(status.candidate || {}, null, 2)}</pre>
        </div>
      </div>
    </div>
  );
